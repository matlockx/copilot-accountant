...
- **Run a single test:** `go test ./path/to/package/...`
...
All OpenCode workflow machinery lives in `.agent/` (hidden, gitignored state) and `specs/` (committed, user-visible).

```
rng-service/
├── AGENTS.md                   ← this file
├── opencode.json               ← workflow configuration
├── specs/                      ← committed to git (human-readable specs)
│   └── ISSUE-1.md
└── .agent/                     ← gitignored workflow machinery
    ├── command/                ← /spec, /createtasks, /implement, /issue
    ├── agent/                  ← AI agent persona definitions
...
Use `/issue`, `/spec`, `/createtasks`, and `/implement` to drive all workflow.
...
- **Backend**: Pluggable workflow engine (`file`, `jira-taskwarrior`, etc.)
- **Issue**: High-level work item (user story, bug, epic) tracked by the backend
- **Spec**: Technical specification document (markdown) derived from an issue — lives in `specs/`
- **Task**: Granular implementation work item created from an approved spec
