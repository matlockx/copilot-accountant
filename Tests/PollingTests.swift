...
struct PollingTests {
    static func main() {
        ...
        test.run("test_Polling_IntervalIsConfigurable") {
            var config = BudgetConfig(monthlyBudget: 300, username: "test", pollingIntervalMinutes: 10, notificationsEnabled: true, alertAt80Percent: true, alertAt90Percent: true, launchAtLogin: false)
        ...
        test.run("test_Polling_ConfigSerialization_PreservesInterval") {
            let original = BudgetConfig(monthlyBudget: 300, username: "test", pollingIntervalMinutes: 15, notificationsEnabled: true, alertAt80Percent: true, alertAt90Percent: true, launchAtLogin: false)
